...
## What it covers
Five axes — **security, edge-case, concurrency, abuse, logic** — plus dependency CVEs, secrets, and IaC / cloud-config, across **Python, Rust, TypeScript, React**. Detection is **assembled** from off-the-shelf scanners (Semgrep, Bandit, Ruff, mypy, Clippy, Trivy, Gitleaks, pip-audit, osv-scanner, eslint) + **107 curated custom rules** (taint-mode dataflow + precise security/logic/IaC; recall-biased proxies retired to `infra/experimental/`), optional CodeQL cross-file taint, and a dynamic logic lane that produces reproducing inputs. **The edge over a bare linter:** every scan also produces a **blind-spot map** (`beacons/BLINDSPOTS.md`) — the regions it *structurally could not analyze* — so coverage gaps are explicit, never silent. Lead your report with the dark zone, not just the beacons.
